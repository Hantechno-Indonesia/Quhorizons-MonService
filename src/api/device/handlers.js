...
class DeviceHandler {
    ...
    async postDeviceHandler (request, h) {
        try {
            ...
            if (typeof insertDevice !== 'undefined' && insertDevice.status === 'failed') {
                const response = h.response({
                    status: 'failed',
                    message: insertDevice.message
                })
                response.code(402)
                return response
            }
    ...
    async getAllSensorStatusByCompanyHandler (request, h) {
        ...
        // get all devices id
        const devices = await this._service.getDeviceByCompanyUid(companyUid, d)
